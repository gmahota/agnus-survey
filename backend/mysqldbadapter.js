...
function MySQLDBAdapter() {
  ...
  function addSurvey(name, callback) {
    ...
    db.query(sql, [uuidv4(), name, "{}"], function (err, result) {
      if (err) throw err;
  ...
  function postResults(postId, json, callback) {
    var sql = "INSERT INTO results (id,postid, json) VALUES(?,?, ?) ";
    db.query(sql, [uuidv4(), postId, json], function (err, result) {
      if (err) throw err;
      db.query(
        "SELECT * FROM results WHERE postid = ?",
        [postId],
        function (err, result) {
          console.log(JSON.stringify(result));
          callback(result);
  ...
  function storeSurvey(id, json, callback) {
    ...
    db.query(sql, [json, id], function (err, result) {
      if (err) throw err;
      console.log(
        "Number of records affected: " + JSON.stringify(result.affectedRows)
      );
      db.query(
        "SELECT * FROM surveys WHERE id = ?",
        [id],
